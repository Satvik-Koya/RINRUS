...
def write_psi4_input(inp_name,charge,multiplicity,pic_atom,tot_charge,res_count,method="b3lyp-d3bj",basis="def2-svp",job="energy",mem_gb=8,threads=8):
    """Write a complete Psi4 input file using RINRUS atom data."""
    if job not in ["energy","opt","freq"]:
        raise ValueError("job must be one of: energy, opt, freq")

    frozen = []
    with open('%s'%inp_name,'w') as inp:
        inp.write('# generated by RINRUS PyMOL->Psi4 converter\n')
        inp.write('# source: %s\n\n'%res_count)
        inp.write('memory %d GB\n\n'%mem_gb)
        inp.write('set_num_threads(%d)\n\n'%threads)
        out_name = os.path.splitext(os.path.basename(inp_name))[0]
        inp.write('set_output_file("%s.out", False)\n\n'%out_name)
        inp.write('molecule mol {\n')
        inp.write('%d %d\n'%(charge+tot_charge,multiplicity))
        inp.write('units angstrom\n')
        inp.write('symmetry c1\n')
        inp.write('no_com\n')
        inp.write('no_reorient\n')
        for i in range(len(pic_atom)):
            atom = pic_atom[i]
            el = atom[14].strip() if atom[14].strip() else atom[2].strip()[0]
            inp.write('%2s %12.6f %12.6f %12.6f\n'%(el,atom[8],atom[9],atom[10]))
            if atom[16] == '-1':
                frozen.append(i+1)
        inp.write('}\n\n')
        inp.write('set {\n')
        inp.write('basis %s\n'%basis)
        inp.write('scf_type df\n')
        inp.write('guess sad\n')
        inp.write('maxiter 200\n')
        inp.write('e_convergence 1e-8\n')
        inp.write('d_convergence 1e-8\n')
        inp.write('soscf true\n')
        if any(x in method.lower() for x in ['b3lyp','pbe','wb97','m06','b97','scan']):
            inp.write('dft_radial_points 99\n')
            inp.write('dft_spherical_points 590\n')
        inp.write('}\n\n')
        if job == 'opt':
            inp.write('set optking {\n')
            inp.write('g_convergence gau_tight\n')
            if len(frozen) > 0:
                fstr = ', '.join(['%d xyz'%x for x in frozen])
                inp.write('frozen_cartesian [%s]\n'%fstr)
            inp.write('}\n\n')
        if job == 'energy':
            inp.write("energy('%s')\n"%method)
        elif job == 'opt':
            inp.write("optimize('%s')\n"%method)
        elif job == 'freq':
            inp.write("frequency('%s')\n"%method)
